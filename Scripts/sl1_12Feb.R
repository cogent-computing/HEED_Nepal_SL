...
############# Read in data for SL1 - read in stitched data till Oct and then onwards ##########
...
colnames(sl_write2) <- c("Time","ID","Actual_PV_power_W","Battery_power_W",
                        "Charged_energy_W","Discharged_energy_W",
                        "PV_DC_coupled_W", "PV_power_W", "State_of_charge_%")
sl1_write <- data.frame()
sl1_write <- rbind(sl1_write, sl_write, sl_write2)
setwd("~/OneDrive - Coventry University/HEED_analysis/Nepal Streetlights/Data/")
write.csv(sl1_write, file="SL1_jul_jan.csv", row.names=FALSE)
...
sl_all_daily[sl_all_daily$streetlight=="SL1",] %>%
  ggplot(aes(x=days, y=load, color="Actual")) +
  geom_point(shape=8) + 
  labs(title="Daily system and socket load at SL1 since commissioning (2019-07-01) " , 
...
act_power_typical <- sl_all %>%
  ...
  summarise(act_power = mean(Actual_power_W))
...
#Select SL1 - Jan data is missing
pot_pv_typical <- pot_pv_typical[pot_pv_typical$streetlight=="SL1", ]
...
#Extract now for SL1
weather_monthly <- weather_monthly[weather_monthly$streetlight=="SL1",]
...
weather_monthly %>%
  ...
  geom_bar(stat="identity", width=.5, position = "stack") + 
  labs(title="Capture losses at Nepal SL1 in 2019" ,
